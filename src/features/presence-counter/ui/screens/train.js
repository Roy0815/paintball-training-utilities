...
export function renderTrainScreen(container, draft, { onDone }) {
  setHeader({ title: t('pc.train.title'), onBack: null });

  container.innerHTML = `
    <section class="screen">
      <h2>${t('pc.train.title')}</h2>
      <p class="hint" id="status">${t('pc.train.running')}</p>
      <progress id="progress" max="1" value="0"></progress>
      <p class="error" id="error-msg" hidden></p>
      <button type="button" class="btn btn-primary" id="done-btn" hidden>${t('pc.train.doneBtn')}</button>
    </section>
  `;
    ...
    try {
      const {
        ...
        onProgress: (done, total) => {
          if (cancelled) return;
          statusEl.textContent = t('pc.train.runningProgress', { done, total });
      ...
      statusEl.textContent = t('pc.train.done', { name: profile.name });
      progressEl.hidden = true;
      doneBtn.hidden = false;
    } catch (err) {
      if (cancelled) return;
      errorMsg.textContent = t('pc.train.error', { message: err.message });
